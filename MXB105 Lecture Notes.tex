...
\begin{equation*}
    a_2\dv[2]{y}{x} + a_1\dv{y}{x} + a_0y = F(x)
\end{equation*}
...
form that it would take. The following table summarises the appropriate
form of $y_P$ based on $F$.
\begin{table}[H]
    \centering
    \begin{tabular}{c | c}
        \toprule
...
\end{equation*}
\begin{description}
    \item[Newton's Law] $F = m \dv[2]{y}{t}$
    \item[Spring force.] $F_s = -k y$
    \item[Damping force.] $F_s = -\gamma \dv{y}{t}$
\end{description}
\begin{equation*}
    m \dv[2]{y}{t} + \gamma \dv{y}{t} + k y = f(t)
\end{equation*}
...
\begin{description}
    \item[Resistor.] $iR$
    \item[Capacitor.] $q/C$
    \item[Inductor.] $L \dv{i}{t}$
\end{description}
where $R$ is the resistance in \si{\ohm}, $C$ is the capacitance in \si{\farad}, and $L$ is the inductance in \si{\henry}.

Kirchhoff's Voltage law states that the sum of voltages around a loop equals 0. 
Therefore in an RLC circuit, with a voltage source supplying $v(t)$,
\begin{align*}

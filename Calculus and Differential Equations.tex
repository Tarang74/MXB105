...
\begin{theorem}[L'H\^opital's Rule]
    ...
    % \begin{equation*}
    %     \lim_{x\to x_0} f(x) = \lim_{x\to x_0} g(x) = 0 \lor \lim_{x\to x_0} f(x) = \lim_{x\to x_0} g(x) = \pm \infty \implies \lim_{x\to x_0} \frac{f(x)}{g(x)} = \lim_{x\to x_0} \frac{f'(x)}{g'(x)}
    % \end{equation*}
\end{theorem}
...
\end{itemize}
    If $f(x)$  is continuous on $\left(-\infty,\:\infty\right)$, $f(x)$ is continuos everywhere.
\end{theorem}
...
\begin{theorem}[The Fundamental Theorem of Calculus: Part 1]
    ...
    \begin{equation*}
        \int_a^b f(x)\dd{x} = F(b) - F(b)
    \end{equation*}
\end{theorem}
\begin{theorem}[The Fundamental Theorem of Calculus: Part 2]
    If $f(x)$ is continuous on $I$ then it has an antiderivative on $I$.
    \begin{equation*}
        \int_a^b f(x)\dd{x} = F(b) - F(b)
    \end{equation*}
\end{theorem}
